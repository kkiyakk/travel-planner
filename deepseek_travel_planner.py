...
class TravelPlanner:
    ...
    def process_request(self, system_prompt: str, user_prompt: str) -> str:
        if not DEEPSEEK_API_KEY:
            st.error("DEEPSEEK_API_KEY가 설정되지 않았습니다. .env 파일 또는 Render 환경변수를 확인하세요.")
            return ""
...
def main():
    ...
    st.markdown(
        """
        Using DeepSeek API to generate travel plans, travel tips, and destination recommendations.
        """
    )
